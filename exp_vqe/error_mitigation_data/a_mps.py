...
class SimpleMPS:
    ...
    def get_theta3(self, i):
        """Calculate effective three-site wave function on sites i,j=(i+1), k=(i+2) in mixed canonical form.

        The returned array has legs ``vL, i, j, k, vR``.
        """
        k = (i + 2) % self.L
        return np.tensordot(self.get_theta2(i), self.Bs[k], [3, 0])  # vL i j [vR], [vL] k vR
