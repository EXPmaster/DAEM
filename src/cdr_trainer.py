...
class CDRTrainer:
    ...
    def fit(self, param, observable):
        ...
        training_circuits = self.generate_training_circuits(
            circuit,
    ...
    @staticmethod
    def generate_training_circuits(circuit, num_training_circuits, **kwargs):
        train_circuits = []
        random_cliffords = ['x', 'y', 'z', 's', 'sdg', 'h', 'sx', 'sxdg']
        for i in range(num_training_circuits):
            ...
            for operation, gate_qubits, _ in circuit.data:
                if operation.name in ['u', 'u3']:
                    # train_circ.append(np.random.choice(random_cliffords), [q.index for q in gate_qubits])
                    eval('train_circ.' + f'{np.random.choice(random_cliffords)}({gate_qubits[0].index})')
            ...
            train_circuits.append(train_circ)
        return train_circuits


def pretrain_swaptest():
    import torch
...
def pretrain_circuits(env_root):
    for circuit_name in os.listdir(env_root):
        param = float(circuit_name.replace('.pkl', '').split('_')[-1])
        circuit_path = os.path.join(env_root, circuit_name)
        self.envs[param] = IBMQEnv(circ_path=circuit_path)
