...
def main(args):
    ...
    if args.miti_prob:
        ...
        loss_fn = nn.MSELoss()
    model = SuperviseModel(num_qubits=args.num_qubits, mitigate_prob=args.miti_prob)
    ...
    for epoch in range(args.epochs):
        ...
        if args.miti_prob and metric > 0.3:
...
def validate(epoch, args, loader, model, loss_fn):
    ...
    for itr, (params, params_cvt, obs, obs_kron, pos, scale, exp_noisy, gts) in enumerate(loader):
        ...
        if args.miti_prob:
            ...
            unmitigated_metric.update(nn.CosineSimilarity()(exp_noisy[:, 0], gts).mean().item())
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-path', default='../data_mitigate/phasedamp/trainset_vqe4l.pkl', type=str)
    parser.add_argument('--test-path', default='../data_mitigate/phasedamp/valset_vqe4l.pkl', type=str)
    ...
    args.logdir = os.path.join(args.logdir, f'env_qaoa6q_new_depo_{time_str}')
